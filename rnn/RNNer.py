...
class RNNer:
    def __init__(
        ...
        X_train, y_train, X_test, y_test = self._read_data(
            data_path, window_size, hindcasting
        )
        ...
        X_train_scaled = self.x_scaler.fit_transform(X_train_reshaped).reshape(
            n_train_samples, window_size, n_features
        )
        X_test_scaled = self.x_scaler.transform(X_test_reshaped).reshape(
            n_test_samples, window_size, n_features
        )
        y_train_scaled = self.y_scaler.fit_transform(y_train).reshape(
            n_train_samples, window_size
        )
        y_test_scaled = self.y_scaler.transform(y_test).reshape(
            n_test_samples, window_size
        )
        ...
        self.y_train_tensor = (
            ...
            if hindcasting
            else torch.tensor(y_train_scaled, dtype=torch.float32)
            .unsqueeze(1)
            .to(device)
        )
        ...
        self.y_val_tensor = (
            ...
            if hindcasting
            else torch.tensor(y_val.reshape(-1, window_size), dtype=torch.float32).to(
                device
            )
        )
    ...
    def _read_data(self, path, window_size, hindcasting):
        ...
        X_test, y_test = (
            X[adjusted_start : adjusted_end + 1],
            y[adjusted_start : adjusted_end + 1],
        )
    ...
    def _create_sliding_windows(self, X, y, window_size, hindcasting):
        ...
        if window_size > n_samples:
            raise ValueError(
                "Window size must be smaller than or equal to the number of samples in X."
            )
        ...
        y_windows = (
            np.array(
                [
                    y[i : i + window_size].flatten()
                    for i in range(n_samples - window_size + 1)
                ]
            )
            if hindcasting
            else y[window_size - 1 :]
        )
    ...
    def _validation_set(self, X_train, y_train, plot_validation=False):
        ...
        season_start = self.season_start - (
            self.window_size - 1
        )  # need to adjust this value for subsequent calculations
