...
class Lassoer:
    def __init__(self, season, alpha=None, model="lasso", advanced_validation=False):
        ...
        if advanced_validation:
            self.X_train_scaled, self.y_train_scaled, self.X_val, self.y_val = (
                self._validation_set(self.X_train_scaled, self.y_train_scaled)
            )
        else:  # last year as validation set
            self.X_val = self.X_train_scaled[
                self.season_start - days : self.season_start
            ]
            self.y_val = self.y_train_scaled[
                self.season_start - days : self.season_start
            ]
        ...
        if model == "lasso":
            ...
            self.tol = 0.0001
            # if no alpha provided, tune for optimal alpha
    ...
    def fit(self, show_chart=True, file=None):
        ...
        self.og_actual = self.y_scaler.inverse_transform(
            self.y_test_scaled.reshape(-1, 1)
        )

        if file:
            pred_df = pd.DataFrame(self.og_actual)
            pred_df.to_csv(file, header=False, index=False)

        print(
            "Mean absolute error: %.2f"
            % mean_absolute_error(self.og_actual, self.og_pred)
        )
        print(
            "Mean squared error: %.2f"
            % mean_squared_error(self.og_actual, self.og_pred)
        )
        print(
            "Correlation: %.2f"
            % np.corrcoef(self.og_actual.reshape(-1), self.og_pred.reshape(-1))[0, 1]
        )
    ...
    def _find_optimal_alpha(self):
        ...
        for a in alphas:
            ...
            zpercs.append(
                ((1000 - zero_weights) / 1000) * 100
            )  # 1000 = size of input data
